...
func TestPlanCmd_CheckPsqlOK(t *testing.T) {
	assert := assert.New(t)
	require := require.New(t)
	...
	cmd := &qrev.PlanCmd{Path: "*.sql", CheckPsql: true}
	err := cmd.Run(options)
...
func TestPlanCmd_CheckPsqlSyntaxErr(t *testing.T) {
	assert := assert.New(t)
	...
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	os.WriteFile("20251010-init-table.sql", []byte("invalid1"), 0400)      // ignore
	os.WriteFile("20251011-update-data.sql", []byte("invalid2"), 0400)     // ignore
	os.WriteFile("20251012-delete-old-data.sql", []byte("invalid3"), 0400) // ignore
	os.WriteFile("20251013-new.sql", []byte("invalid4"), 0400)
